    ...
    public class JobDriver_FollowClose : JobDriver
	{
		...
		public override IEnumerable<Toil> MakeNewToils()
		{
			...
			toil.tickAction = delegate
			{
				...
				if (!pawn.pather.Moving || pawn.IsHashIntervalTick(30))
				{
                    ...
                    if (!pawn.pather.Moving || !pawn.pather.Destination.Cell.InHorDistOf(lastPassableCellInPath, followRadius))
                    {
		...
		public IntVec3 GetCellToFollow(Pawn target)
        {
			if (target.pather.Moving && target.pather.curPath != null)
            {
                ...
                while (maxValue > 0)
                {
					var nodes = target.pather.curPath.NodesReversed;
                    if (nodes.Count > maxValue)
                    {
